...
class AgentConversationService:
    ...
    def chat(self, user, message: str, thread_id: Optional[str] = None) -> Dict:
        ...
        if pending_action is not None:
            noun = (
                "incidencia"
                if pending_action.tipo_accion == "INCIDENCIA_CREAR"
                else "reserva"
            )
            return {
                "thread_id": public_thread_id,
    ...
    def resume_confirmation(self, user, action: AgentAction, approved: bool) -> Dict:
        ...
        if approved and action.estado == AgentAction.EJECUTADA:
            ...
            is_incident = action.tipo_accion == "INCIDENCIA_CREAR"
            reference = result.get("incident_id") if is_incident else result.get("reservation_id")
            backend_status = (
                result.get("incident_status")
                if is_incident
                else result.get("reservation_status")
            )
            noun = "Incidencia" if is_incident else "Reserva"
            return {
                ...
                "intent": "incident" if is_incident else "reservation",
                "status": "ok",
                "error": None,
                "checkpoint_backend": self.runtime.backend,
                "durable": self.runtime.durable,
        ...
        response["intent"] = (
            "incident" if action.tipo_accion == "INCIDENCIA_CREAR" else "reservation"
        )
        self._record_trace(response)
        return response
        ...
        metadata.update(
            {
                ...
                "action_type": (
                    "RESERVA_CREAR"
                    if response.get("intent") == "reservation"
                    else "INCIDENCIA_CREAR"
                    if response.get("intent") == "incident"
                    else ""
                ),
            }
        )
